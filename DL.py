...
Based on DL.py from adef17286-sudo: https://github.com/adef17286-sudo/NPO_start_DL
"""
...
def download(link, save_name):
    ...
    if not Path(provision_path).is_file():
        return f"[!] Provisioning file '{provision_path}' not found."
        
    out, err, code = run_npo_get_output(link)
    if code != 0:
        return "[!] NPO.py exited with code"

    combined = out + "\n" + err
    mpd_url = extract_mpd_url(combined)
    if not mpd_url:
        return "[!] Could not find .mpd URL in output."

    drm_token = extract_drm_token(combined)
    if not drm_token:
        return "[!] Could not find DRM token."
    ...
    if not psshs:
        print("[!] No suitable PSSH blocks found. Deze video is waarschijnlijk niet DRM-beveiligd.")
    else:
        all_keypairs = []
        for pssh in psshs:
    ...
    cmd += ["--save-dir", "download", "-dv", "all", "-sa", "best", "-M", "format=mp4"]
...
def main():
    ...
    splitter = link.split("/")
    save_name = splitter[5] + "--" +  splitter[7] + "--" + splitter[8]
    ...
    # Append fixed options
    cmd += ["-dv", "all", "-sa", "best", "-M", "format=mp4"]
...
if __name__ == "__main__":
    resultaten = []
    with open("alle_series_afleveringen_resultaten.csv") as f:
        ...
        for row in records:
            ...
            print(f"\nDownloading episode: {episode_url}")
            if resultaat != "gedaan" and resultaat != "[!] Could not find .mpd URL in output.":
                try:
                    resultaat = download(episode_url, save_name)
                except Exception as e:
                    resultaat = f"[!] Fout tijdens downloaden: {e}"
                print(resultaat)
                row["resultaat"]  = resultaat
            else:
                print(f"  {save_name} al gedownload, overslaan.")
            resultaten.append(row)
    
    with open("resultaten_dl.csv", "w", newline="", encoding="utf-8") as f:
        fieldnames = ["series_url", "season_url", "episode_url", "save_name", "resultaat"]
        ...
        try:    
            ...
            with open("output.txt", "w", encoding="utf-8") as f:   
                for resultatenrij in resultaten:
                    for key, value in resultatenrij.items():
                        f.write(f"{key}:{value}\n")
